...
public class BlockCampfire extends Block implements IBlockStateIgnore {
    ...
    public boolean onBlockActivated(World world, BlockPos pos, IBlockState state, EntityPlayer player, EnumHand hand,
        ...
        if (OreDictUtil.isOre("logWood", heldItemStack)) {
            ...
            if (ignited) {
                // FORCE RESCHEDULE: Ensure the update loop handles raining instantly when lit
                // manually
                scheduleRainCheck(world, pos);
            }
            return true;
    ...
    public void randomDisplayTick(IBlockState state, World world, BlockPos pos, Random rand) {
        ...
        if (state.getValue(BURNING)) {
            ...
            if (rand.nextInt(30) == 0) {
                world.playSound(0.5d + pos.getX(), 0.5d + pos.getY(), 0.5d + pos.getZ(),
                        SoundEvents.BLOCK_FURNACE_FIRE_CRACKLE, SoundCategory.BLOCKS, 0.5f, 1.0f, false);
            }
        }
    }
    ...
    private void createFlameParticle(World world, BlockPos pos, Random rand) {
        ...
        world.spawnParticle(EnumParticleTypes.FLAME, xOffset + pos.getX(), yOffset + pos.getY(), zOffset + pos.getZ(),
